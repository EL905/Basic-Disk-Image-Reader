...
# Date: 10/03/2021
# About: It takes disk image file, then it reads info about partitions, fat volume and ntfs and displays them in graphical format
################################
    ...
    with open(filename, "rb") as f:
        ...
        for i in range(0, int(rt_dir_size*512),  32):
            ...
            if file_title[0] == 229:
                ...
                result +="\nAddress of starting cluser for that file in decimal: " + str(int.from_bytes(file_title[26:28], byteorder='little'))
                csa = cluser2_start + ((int.from_bytes(file_title[26:28], byteorder='little')-2)*8)
                file_content = txt[(int(csa)*512)-(partition_start*512):(int(csa)*512)-(partition_start*512)+16]
                result +="\nFirst 16 bytes of content of that file: " + str(file_content)+"\n--------------------------------------------------------------"
                break
...
def ntfs_info(filename, partition_start):
    with open(filename, "rb") as f:
        ...
        return "NTFS Info(all in decimal):\nBytes per sector is: " + str(bytes_per_sector) + "\nSectors per cluster is: "+str(sectors_per_cluster) + "\nMFT sector address: "+str(mft_sector_address) + "\nFirst attribute offset: "+str(first_attribute_offset)+"\nFirst attribute: "+str(first_attribute)+"\nFirst attribute length: "+str(first_attribute_length) + "\nSecond attribute offset: "+str(second_attribut_offset) + "\nSecond attribute: "+str(second_attribute)+"\nSecond attribute length: "+str(second_attribut_length)
...
def upload_file():
    ...
    display_info(txt) # in ealier versions this method was called at the very end (after for loop ends), but I discovered txt looks nicer that way
    display_info("--------------------------------------------") # this makes a nice line under partition info so its easier to read other info
    if ntfs_exists:
